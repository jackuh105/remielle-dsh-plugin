...
export function apply(ctx) {
  ctx.inject(["sessionProjections"], (projectionCtx) => {
    projectionCtx.sessionProjections.register({
      key: "remielle",
      // DSH 0.1.1-rc.x renamed the state schema field `schema` → `stateSchema`,
      // and moved the client-visible view under `wire: { viewSchema, view }`.
      // Without `wire` the unit is host-only and never reaches the browser
      // (snapshot/viewCheckpoint/restore all skip units whose `wire` is undefined),
      // so the pet would never see phase changes.
      stateSchema: remielleSchema,
      init: () => ({ phase: "idle" }),
      apply: applyRemielle,
      wire: {
        viewSchema: remielleSchema,
        view: (state) => state,
      },
      stateVersion: 1,
    });
